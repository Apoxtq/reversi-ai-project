...
1. **对话交流必须使用中文** 🇨🇳
   ...
   - ✅ **问题讨论、建议** → 中文
   - ✅ **文档展示（在聊天中）** → 中文
   - 📌 **原因**：用户是中国学生，中文方便理解和快速沟通
   
2. **项目代码和文档必须使用纯英文** 🇬🇧
   - ✅ **所有代码文件** (.cpp, .h) → 纯英文
   - ✅ **代码注释** → 英文
   - ✅ **变量名、函数名、类名** → 英文
   - ✅ **Git 提交信息** → 英文（或中文，视情况而定）
   - ✅ **技术文档** (如果上传GitHub) → 英文
   - 📌 **原因**：这是英国大学项目，必须符合学术规范
   
3. **本地文档可以使用中文** 📝
   - ✅ `project_docs/` 中的文档 → 中文（不上传GitHub）
   - ✅ `docs/` 中的学习笔记 → 中文
   - ✅ 个人笔记和计划 → 中文
   - ❌ `project_docs/github/` 中的文档 → 英文（会上传GitHub）

**示例对比：**

```markdown
✅ 正确：聊天回复用中文
AI: "我已经修复了README中的所有失效链接，删除了11个不存在的文档引用..."

❌ 错误：聊天回复用英文
AI: "I have fixed all broken links in README..."
...
// bitboard.h
class Bitboard {
    uint64_t black_pieces;  // Black player pieces
    uint64_t white_pieces;  // White player pieces
};
...
✅ 正确：Git提交信息（英文更专业）
git commit -m "fix: Remove broken documentation links from README"

✅ 也可以：Git提交信息（中文也可接受）
git commit -m "fix: 修复README中所有失效的文档链接"
```

**重要提醒：**
- 🚨 **项目代码禁止出现中文** - 这会影响学术诚信
- 🚨 **对话必须用中文** - 这样用户才能快速理解
- 🚨 **GitHub公开文档用英文** - 这是国际标准
- ✅ **本地笔记用中文** - 这是私人学习记录
...
**第三步：** 记住核心原则（优先级从高到低）
1. **语言规范** → 对话用**中文**🇨🇳，代码/GitHub文档用**英文**🇬🇧
2. 总结报告 → **聊天中展示**，不创建文件
3. 工作方式 → **主动执行**，批量操作
4. 文档更新 → **同步所有**相关文档
5. Git 操作 → 详细提交信息，**不自动推送**
